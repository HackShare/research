...
if [ $curUser = "root" ]; then
	...
	export testExists=$(sudo docker ps -a | grep "RMI-Example"* | awk '{print $NF}')
	if [[ $testExists = "RMI-Example"* ]]; then	#A container is running
		echo "Stopping containers..."
		docker stop $(sudo docker ps -a | grep "RMI-Example"* | awk '{print $NF}')
		echo "Removing stopped containers..."
		docker rm $(sudo docker ps -a | grep "RMI-Example"* | awk '{print $NF}')
	else
	...
	export ImplIP=$(sudo docker inspect $(sudo docker ps -a | grep "RMI-Example-Impl" | awk '{print $NF}') | grep IPAddress.: | awk {'print $2'} | cut -d '"' -f2)
	...
	echo "Not logged in as root. Try sudo bin/run.sh"
fi
